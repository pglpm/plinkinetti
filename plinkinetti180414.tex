...
%% Last-Updated: 2018-04-14T18:10:17+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
proportional to $\prod_i{\yq_i}^{\yN_i}$ for some values of $\yN_i>0$. This
prior is determined by the additional assumption -- call it $\yMJ$ -- that
that the frequencies of other outcomes are irrelevant for predicting a
particular one:
\begin{equation}
  \label{eq:sufficientness}
  \pf(d_{m+1} = k \| \yf, \yMJ) = \pf(d_{m+1} = k \| \yff_{k}, \yMJ)
...
\begin{equation}
  ...
  \int_{\simpl} \prod_{i=1}^N {\yqq_{i}}^{\yN_i}\,\di\yq = \frac{\prod_i \Gamma(\yN_i)}{\Gamma\bigl(\sum_i\yN_i\bigr)}.
\end{equation}
