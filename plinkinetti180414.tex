...
%% Last-Updated: 2018-06-30T01:05:38+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
\citep{iso1993_r2009,iso2006}, except for the use of a comma to denote
logical conjunction \enquote{$\land$}, for simplicity.
...
We shall consider a robot programmed with either of these two equivalent
assumptions:
\begin{itemize}
...
Infinite exchangeability determines this form of the
probability~\eqref{eq:distr_statmodel}:
\begin{equation}
...
probability, for each $m$,
\begin{equation}
  \label{eq:general_prediction}
  \p(D^{m}_{d_{m}} \|
  D^{m-1}_{d_{m-1}}, \dotsc,  D^2_{d_2}, D^1_{d_1},  M),
\end{equation}
as calculated by the robot:
\begin{subequations}\label{eq:prediction_exchangeability}
  \begin{gather}
    \p(D^{m}_{d_{m}}\| D^{m-1}_{d_{m-1}}, \dotsc,  D^2_{d_2}, D^1_{d_1}, M)
    = \int \yqq_{d_{m}} \pf(\yq \| D^1_{d_1}, \dotsc, D^{m-1}_{d_{m-1}}, M)\,\di\yq,
    \\
    \shortintertext{with}
    \pf(\yq \| D^1_{d_1}, \dotsc, D^{m-1}_{d_{m-1}}, M)
    = \frac{ \bigl( \prod_{i=1}^{m-1} \yqq_{d_i} \bigr)\, \pf(\yq \| M)
      }{
      \int  \bigl( \prod_{i=1}^{m-1} \yqq_{d_i}' \bigr)\, \pf(\yq' \| M)\,\di\yq'
      }.
...
\begin{subequations}\label{eq:prediction_exchangeability_example}
  \begin{gather}
    \p(D^5_{19} \| D^1_{37} , D^2_{6} , D^3_{25}, D^4_{37}, M)
    = \int \yqq_{19}\; \pf(\yq \|  D^1_{37} , D^2_{6} , D^3_{25}, D^4_{37}, M)\,\di\yq,
    \\
    \pf(\yq \|  D^1_{37} , D^2_{6} , D^3_{25}, D^4_{37}, M)
