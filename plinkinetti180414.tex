...
%% Last-Updated: 2018-07-01T11:13:11+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
Owing to the special mathematical form of update formulae above, the
\enquote{beliefs} of this robot right after the $(m+1)$th observation, with
outcome $d_{m+1}$, can be represented by parameters
$\bigl( \yNo{m+1}, \yno{m+1} \bigr)$ that are updated recursively from
those right after the previous observation,
$\bigl( \yNo{m}, \yno{m} \bigr)$:
\begin{equation}
  \label{update_params_JD}
  \begin{aligned}
    \yNo{0}&\defd\yN,
    &    \yNo{m+1} &\defd \yNo{m}+1,
    \\
    \ynno{0}_d &\defd\ynn_d,
    & \ynno{m+1}_d &\defd
                        \frac{\yNo{m}}{\yNo{m}+1}\ynno{m}_d +
    \begin{dcases}
      \frac{1}{\yNo{m}+1} &\text{for $d=d_{m+1}$},
      \\
      0 &\text{for $d\ne d_{m+1}$}.
    \end{dcases}
  \end{aligned}
...
prediction bar under slot $d_{m+1}$, leaving the others untouched; or
lowering the prediction bar for all other slots in the same proportion,
leaving the $d_{m+1}$ untouched.
