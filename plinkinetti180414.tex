...
%% Last-Updated: 2018-06-30T16:47:41+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
Plinko experiments \citep{filipowiczetal2014,filipowiczetal2016} it has
$40^{200}-1 \approx 3\times10^{320}$ dimensions.
...
\subsection{The first Bayesian robot}
\label{sec:bayes_robot}

We first consider a robot programmed with either of these two equivalent
assumptions:
\begin{itemize}
\item The joint probability distribution~\eqref{eq:distr_statmodel} for
  ...
  of their order. That is, the order of the observations is irrelevant for
  making inferences.
\item Only the relative frequencies of past observations are relevant for
  making inferences. Any additional data about known observations is
  irrelevant and can be discarded.
\end{itemize}
In either of these assumptions it's implicit that probability distributions
for different number of observations must of course be consistent with one
another through marginalization.

These two equivalent assumptions are called \emph{infinite
...
\citey[\sect~4.2]{bernardoetal1994_r2000}.

A theorem by de~Finetti \parentext{references above} shows that the
assumption of infinite exchangeability implies that the probability
distribution~\eqref{eq:distr_statmodel} must have this mathematical form,
for any $m$:
\begin{equation}
  \label{eq:exchangeability_form}
  \p(D^1_{d_1} , D^2_{d_2} , \dotsc , D^m_{d_m} \| \yI) =
  \int_\simpl   \Biggl( \prod_{i=1}^m \yqq_{d_i} \Biggr)\, \pf(\yq \| \yI)\,\di\yq,
\end{equation}
...
\label{eq:simplex}
\end{equation}
and $\pf(\yq \| \yI)\,\di\yq$ is a normalized density function, called
...
programmed. In fact, the resulting robot is more powerful than necessary,
because it can make inferences about \emph{any} number of unknown
observations, future or past ones, beyond the $M$ considered in our
context.
...
leads to some characteristic features of the robot's inferences and of
their updates:
\begin{itemize}
\item The robot's inferences can be interpreted in several ways. One is
...
\item As data accumulate, the robot's probabilities for the next outcome
  ...
  the form of the prior $\pf(\yq \| \yI)\,\di\yq$ -- unless the latter is
...
\begin{equation}
  ...
  \pf(D^1_{k} \| \yI) \equiv \int \yqq_{k}\; \pf(\yq \| \yI)\,\di\yq,
\end{equation}
but not their prior $\pf(\yq \| \yI)\,\di\yq$.
