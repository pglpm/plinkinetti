...
%% Last-Updated: 2018-07-14T13:32:31+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
Our new robot and its flexibility in representing beliefs can be used in
many different and for different purposes. What to do with them?

A first question is whether the new robot can reproduce some of the
...
this defines a subset of possible robots. Among this subset we then find
the robots that best mimic a particular participant's behaviour. Let's
...
\item The choice of a low-dimensional subset of changepoint function is
 ...
 \item For each chosen participant, the measured discrepancy only refers to
   ...
   experiment were repeated, with a new sequence of observations. This is a
   cause of the multi-optimality just mentioned. In fact it would be
...
\mynote{Luca: to be finished}

to tune the stubbornness parameter $\yN$ and the
...
-- makes the space of probability distributions into a metric space. Given
two probability distributions $\pf(\yO_{s'} \|\yI')=f(s')$,
$\pf(\yO_{s''} \|\yI'')=g(s'')$ and a distance $d(s'-s'')$ in the $s$
space, their distance $K(f,g)$ is defined as follows:
\begin{enumerate*}[label=(\emph{\arabic*})]
\item consider all possible joint distributions $j(s',s'')$ having the two
  distributions as marginals;
\item calculate the expected value of the distance $d(s'-s'')$
  according to all joint distributions from the previous step;
...
Kantorovich distance $K(f,g)$has a simple expression \citep{vallender1972}
