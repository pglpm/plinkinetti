...
%% Last-Updated: 2018-07-03T13:28:43+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
\begin{table}[!p]
  ...
  \begin{tabularx}{\textwidth}{X}\hline
    \begin{enumerate}
      ...
      %   \[ B_m(t) \defd \p(R^{m}_{t} \| \varOmicron^{m-1}_{s_{m-1}} \dotsc,
      %     \varOmicron^{1}_{s_{1}}, \yMc),\qquad 0\le t \le m-1 \]
...
B_{m+1}(r,s_{m+1}) \defd \p(\varOmicron^{m+1}_{s_{m+1}} \| R^{m+1}_{r}, \varOmicron^{m}_{s_{m}},
     ...
     where $\yff_{s_{m+1}}$ is the relative frequency of outcome $s_{m+1}$ in the previous $r$ observations
      ...
      %   \[ A_m(t) \defd \p(\varOmicron^{m}_{s_{m}} \| R^{m}_{t}, \varOmicron^{m-1}_{s_{m-1}}
      %     \dotsc, \varOmicron^{m-t}_{s_{m-t}}, \yMc)
      %   \]
  \end{enumerate}
\item For $s_{m+1} \in \set{1,\dotsc,N}$, calculate
  \[ \begin{multlined}[][0.85\textwidth]
...
$R^{m}_{t}$, is relevant in any case, because the next observation can only
start a new subsequence or continue the previous one, so that either $r=0$
or $r=t+1$:
\begin{equation}
  \label{eq:R_cond_R_simple}
  \p(R^{m+1}_{r} \| R^{m}_{t}, \yMc) =
  \yrs(t,m)\,\delt_{r,0} + [1-\yrs(t,m)]\,\delt_{r,t+1} =
  \begin{cases}
    \yrs(t,m) & \text{for $r=0$},
    \\
    1-\yrs(t,m) &\text{for $r=t+1$},
    \\
    0 & \text{otherwise},
  \end{cases}
\end{equation}
...
$t=r-1$ survives. In our study we assume that $\yrs(t,m)$ either is
independent of $t$ or increases with $t$, expressing our expectation that
...
% calculated recursively by introducing $R^{m}_{t}$ for the previous
% observation:
% \begin{equation}
%   \label{eq:prob_R_and_last_decompose2}
%     \p(R^{m+1}_{r}\| \varOmicron^{m}_{s_{m}}, \dotsc, \varOmicron^{1}_{s_{1}}, \yI) 
%   % &=\sum_{t=0}^{m-1}
%   % \p(R^{m+1}_{r},R^{m}_{t},\|
%   % \varOmicron^{m}_{s_{m}}, \dotsc, \varOmicron^{1}_{s_{1}}, \yI)
%   % \\
% =\sum_{t=0}^{m-1}\p(R^{m+1}_{r} \| R^{m}_{t}, \yI)
%   \times
%   \p(R^{m}_{t} \|
...
% %   =\sum_{t=0}^{m-1}\p(R^{m+1}_{r} \| R^{m}_{t}, \yI)
% %   \times{}\\
% %   \p(\varOmicron^{m}_{s_{m}} \| R^{m}_{t},
% %   \varOmicron^{m-1}_{s_{m-1}} \dotsc, \varOmicron^{m-t}_{s_{m-t}}, \yI)
% %   \times{}\\
% %   \p(R^{m}_{t} \|
% %   \varOmicron^{m-1}_{s_{m-1}} \dotsc, \varOmicron^{1}_{s_{1}}, \yI).
% %           \end{multlined}
