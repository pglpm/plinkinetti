...
%% Last-Updated: 2018-04-15T01:48:26+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
\newcommand*{\propertitle}{Bayesian Plinko}
...
assumptions by a proposition $M$ -- surely a proposition very difficult to
express in writing -- and the $i$th observation by
\(D_i \defd \text{\enquote{The $i$th observation is $d_i$}}\), with
$d_i \in \set{1,\dotsc N}$. Then $M$ allows us to give a numeric value to
\begin{equation}
  \label{eq:general_prediction}
  \p(D_{m+1} \| D_1 \land D_2 \land \dotsb \land D_m \land M),
\end{equation}
with

The proposition $D_i$
In the following we abbreviate logical conjunction \enquote{$\land$} with a
comma, for simplicity. Our statistical terminology and notation follow ISO
standards \citep{iso1993_r2009,iso2006} otherwise.

We shall consider a robot who uses an \emph{infinitely exchangeable} model.
This model, introduced by de~Finetti
...
\begin{equation}
  ...
  \p(d_1=37 , d_2=6 , d_3 =25, d_4=37 \| M) =
...
\begin{subequations}\label{eq:prediction_exchangeability}
  \begin{gather}
    \p(D_{m+1}\| D_1, \dotsc , D_m , M)
    = \int \yqq_{d_{m+1}} \pf(\yq \| D_1, \dotsc, D_m, M)\,\di\yq,
    \\
    \pf(\yq \| D_1, \dotsc, D_m, M)
...
\begin{subequations}\label{eq:prediction_exchangeability_example}
  \begin{gather}
    ...
    \p(d_5=6 \| d_1=37 , d_2=6 , d_3 =25, d_4=3, M)
    ={}\\ \int \yqq_{6}\; \pf(\yq \|  d_1=37 , d_2=6 , d_3 =25, d_4=3, M)\,\di\yq,
  \end{multlined}
    \\
    \pf(\yq \|  d_1=37 , d_2=6 , d_3 =25, d_4=3, M)
...
\begin{equation}
  ...
  \pf(d_1= k \| M) \equiv \int \yqq_{k}\; \pf(\yq \| M)\,\di\yq,
\end{equation}
but not their prior $\pf(\yq \| M)\,\di\yq$.

As a first exploration we consider a \emph{Johnson-Dirichlet} prior,
proportional to $\prod_i{\yq_i}^{\yN_i}$ for some values of $\yN_i$:
\begin{equation}
  \label{eq:JD_prior}
  \pf(\yq \| \yMJ) =
  \frac{\Gamma\bigl(\sum_i\yN_i\bigr)}{\prod_i \Gamma(\yN_i)}\,
  \prod_{i=1}^N{\yq_i}^{\yN_i-1}, \qquad \yN_i>0
\end{equation}
...
\begin{equation}
  ...
  \p(d_{m+1} = k \| \yf, \yMJ) = \p(d_{m+1} = k \| \yff_{k}, \yMJ)
  \qquad k\in\set{1,\dotsc, N},
\end{equation}
where $\yf$ is the $N$-tuple of observed frequencies. This assumption is
called \enquote{sufficientness}
\citep{johnson1924,johnson1932c}[\chap~4]{good1965}{zabell1982,jaynes1986d_r1996}.
...
\begin{equation}
  ...
  \int_{\simpl} \prod_{i=1}^N {\yqq_{i}}^{\yN_i-1}\,\di\yq = \frac{\prod_i \Gamma(\yN_i)}{\Gamma\bigl(\sum_i\yN_i\bigr)}.
\end{equation}

We obtain for the initial distribution:
\begin{gather}
  \label{eq:initial_predictive_JD}
  \p(d_1= k \| \yMJ) = \int \yqq_{k}\; \pf(\yq \| \yMJ)\,\di\yq
  = \frac{\yN_k}{\sum_i\yN_i},
  \\
  \shortintertext{and for the updated density:}
  \pf(\yq \| D_1, \dotsc, D_m, \yMJ)
  = 
  \frac{\Gamma\bigl(\sum_i\yN_i'\bigr)}{\prod_i \Gamma(\yN_i')}\,
  \prod_{i=1}^N{\yq_i}^{\yN_i'-1} \quad\text{with } \yN_i' =\yN_i+ \yff_i.
...
just by making the parameters $\yN_i$ proportional to such probabilities.
The proportionality factor $\yN \defd \sum_i\yN_i$ is left arbitrary. The
larger $\yN$, the more peaked the prior, and therefore the more
reluctant the robot will be in revising its guesses in the light of new
observations.

What happens is this. Suppose that after some observation the predictive
distribution is $\yN_i/\yN$, and that the next outcome is $k$. Then the
probability for slot $k$ is updated to $\frac{\yN_k+1}{\yN+1}$, and that
for all other slots $j$ to $\frac{\yN_j}{\yN+1}$. This update corresponds
to a participant's raising the bar assignment under slot $k$, leaving the
others untouched, and/or lowering the bar assignments for \emph{all} other
slots by the same proportion.

A participant who, after observing outcome $k$, raises the bar under that
slot \emph{and nearby bars} is therefore not acting according to a
