...
%% Last-Updated: 2018-07-01T16:32:23+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
This new robot, denoted $\yMc$, assumes that the sequence of observations
is divided into subsequences of contiguous observations. These subsequences
can have different lengths. Within each subsequence, the order of the
observations doesn't matter. This robot therefore uses an infinitely
exchangeable model with a Johnson-Dirichlet parameter density for each
subsequence, as in the previous study. Only observations within a
subsequence contribute to the update of the distributions for the other
observations in that subsequence. Observations from other subsequences are
ignored. As one subsequence ends, the robot's initial-distribution
parameter $\yn$ and stubbornness $\yN$ are reset.
...
With these propositions, the reset rules described above correspond to
these probability equalities, for all $m$:
\begin{multline}
...
\begin{equation}
  ...
  \p(R^{m}_{r} \| D^{m-1}_{d_{m-1}}, \dotsc, D^{1}_{d_{1}}, \yMc)
\end{equation}
for all $m$, $0\le r\le m-1$, and outcomes $d_i$. Additional assumptions
are necessary to determine these probabilities. For example, the particular
order of all past outcomes, the robot's previous distributions, and other
factors external to the experiment might all be relevant. The robot is
...
\mynote{Luca: work in progress in the rest of this subsection}
...
\begin{multline}
  \label{eq:irrelevant_data_conditional_R}
  \p(D^{m+1}_{d_{m+1}} \|
  R^{m+1}_{r},  D^{m}_{d_{m}}, \dotsc, D^{1}_{d_{1}}, \yMc)
  ={}\\
  \shoveright{\begin{cases}
  \p(D^{m+1}_{d_{m+1}} \|
    R^{m+1}_{r},  D^{m}_{d_{m}}, \dotsc, D^{m+1-r}_{d_{m+1-r}}, \yMc)
    &\text{if $1\le r\le m$},
      \\[2\jot]
  \p(D^{m+1}_{d_{m+1}} \|
    R^{m+1}_{r}, \yMc)
...
\begin{multline}
  ...
  \p(D^{m+1}_{d_{m+1}} \|
  D^{m}_{d_{m}}, \dotsc, D^{1}_{d_{1}}, \yMc)
  =
...
=  {}\\[2\jot]
  ...
  R^{m+1}_{r},  D^{m}_{d_{m}}, \dotsc, D^{m+1-r}_{d_{m+1-r}}, \yMc)
  \times
  \p(R^{m+1}_{r}, D^{m}_{d_{m}} \| D^{m-1}_{d_{m-1}} \dotsc, D^{1}_{d_{1}}, \yMc)}{
\p(D^{m}_{d_{m}} \| D^{m-1}_{d_{m-1}} \dotsc, D^{1}_{d_{1}}, \yMc)
}.
\end{multline}
...
\p(D^{m-1}_{d_{m-1}} \| D^{m-2}_{d_{m-2}}, \dotsc, D^{1}_{d_{1}}, \yMc)}
  ...
  \begin{aligned}
    \frac{1}{\p(D^{m-1}_{d_{m-1}} \| D^{m-2}_{d_{m-2}}, \dotsc, D^{1}_{d_{1}}, \yMc)}
  \sum_{s=0}^{m-1} \p(R^{m+1}_{r} \| R^{m}_{s}, \yMc)
  \times{}\qquad\qquad&\\[\jot]
  \p(D^{m}_{d_{m}} \|
    R^{m}_{s}, D^{m-1}_{d_{m-1}} \dotsc, D^{m-s}_{d_{m-s}}, \yMc)
    \times{}\qquad&\\[\jot]
...
\begin{equation}
  \label{eq:prob_R_and_last_decompose}
  \p(R^{m+1}_{r} \|
  R^{m}_{s},  D^{m}_{d_{m}} \dotsc, D^{1}_{d_{1}}, \yMc)
...
For $m=0$ we have that $\p(D^{1}_{d_1} \| \yMc)$ is given by the
Johnson-Dirichlet model, and $\p(R^{1}_{0} \| \yMc)=1$ because the first
...
$\p(D^{m+1}_{d_{m+1}} \| D^{m}_{d_{m}}, \dotsc, D^{1}_{d_{1}}, \yMc)$ is
