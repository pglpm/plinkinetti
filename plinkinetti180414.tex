...
%% Last-Updated: 2018-07-03T13:45:36+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
construct such a robot $\yIp$ we just need to set its distribution for the
$m$th observation conditional on the observed $s_1,\dotsc,s_{m-1}$, that
is,
$\p(\varOmicron^m_{s_m} \|
\varOmicron^{m-1}_{s_{m-1}},\dotsc,\varOmicron^1_{s_1},\yIp)$, equal to the
participant's~\eqref{eq:cond_distribs}, for all $m$; and then
\emph{arbitrarily} choose the distributions conditional on all other
outcome combinations that haven't been observed. By construction these
conditional probabilities give a properly normalized joint
distribution~\eqref{eq:distr_statmodel}. The simplest such robot is the one
for which
\begin{subequations}
