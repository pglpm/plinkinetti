...
%% Last-Updated: 2018-05-28T18:21:33+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
% We can rewrite the above probability also as
% \begin{multline}
%   \label{eq:prob_new_cond_R_cond_last}
%   \p(D^{m+1}_{d_{m+1}} \|
%   D^{m}_{d_{m}}, \dotsc, D^{1}_{d_{1}}, \yI)
%   ={}\\[\jot]
%   \frac{\sum_{r=0}^m
...
% observation, which we can have saved into memory. The joint probability for
% $R^{m+1}_{r}$ and $D^{m}_{d_{m}}$ in the sum can be decomposed introducing
% $R^{m}_{s}$ for the previous observation:
%
% \begin{multline}
...
subsequence is. It can be calculated recursively by introducing $R^{m}_{s}$
...
%   \end{aligned}
\end{equation}
Let's examine the two factors in the last sum.
...
The second factor in~\eqref{eq:prob_R_and_last_decompose} is a distribution
analogous to the one calculated there, but for the previous observation. In
our recursive scheme it was already calculated in the previous observation
...
\begin{table}[!b]
  ...
  \begin{tabularx}{\textwidth}{X}\hline
    ...
    \item\label{item:first_step}For $0\le r \le m$:
      ...
      \item Calculate
        \[ \begin{multlined}[t][0.8\textwidth] B_{m+1}(r) \defd
            \p(R^{m+1}_{r} \| D^{m}_{d_{m}}, \dotsc,
            D^{1}_{d_{1}}, \yI) ={}\\
        \begin{cases}
          \sum_{s=0}^{m-1} \yrs(s) \times B_m(s) & \text{if $r=0$},
          \\
          [1-\yrs(r-1)]  \times B_m(r-1) & \text{if $r\ge 1$}    
        \end{cases}
      \end{multlined}
    \]
  % \item retrieve from memory
  %   \[ C_m \defd \p(D^{m}_{d_{m}} \| D^{m-1}_{d_{m-1}} \dotsc, D^{1}_{d_{1}},
  %     \yI) \]
  \end{enumerate}
\item Calculate
  \[ \p(D^{m+1}_{d_{m+1}} \| D^{m}_{d_{m}}, \dotsc, D^{1}_{d_{1}}, \yI) =
    \sum_{r=0}^{m} A(r) \times B_{m+1}(r)\]
\item Save $B_{m+1}(r)$ into memory
