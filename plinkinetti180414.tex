...
%% Last-Updated: 2020-01-12T16:19:12+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
\newcommand*{\propertitle}{Bayesian Plinko:\\ Bayesian models to mimic human inferences [draft]}
\newcommand*{\pdftitle}{Bayesian Plinko}
\newcommand*{\headtitle}{Bayesian Plinko}
\newcommand*{\pdfauthor}{P.G.L.  Porta Mana, A. Filipowicz}
\newcommand*{\etal}{{et al.}}
\newcommand*{\headauthor}{\ifpublic Porta Mana \amp\ Filipowicz%
\else Alex \etal\fi}
...
\subsection{What goes on in a participant's head? Possible models}
...
\subsection{Paradigms for judgement and assessment}
...
states that the $m$th observation is the start of a new subsequence.

For fixed $m$, the propositions $R^{m}_{u}$, $0\le u \le m-1$, are
obviously exhaustive (their probabilities sum to unity) and mutually
exclusive. The assumptions $\yMc$ made thus far imply that two propositions
for consecutive $m$ are also exclusive and exhaustive:
\begin{subequations}
  \label{eq:exhaustive_R_from_previous}
  \begin{gather}
    \p(R^{m}_{u_{m}}, R^{m-1}_{u_{m-1}} \| \yMc) = 0
    \text{\quad if\quad$u_{m}\ne 0$ and $u_{m} \ne u_{m-1}+1$},
    \\\intertext{and therefore}
    \p(R^{m}_{0} \| R^{m-1}_{v-1},\yMc) +
    \p(R^{m}_{v} \| R^{m-1}_{v-1},\yMc) = 1.
  \end{gather}
\end{subequations}
...
\begin{equation}
  ...
  \p(R^{m}_{u_{m}} \|  R^{m-1}_{u_{m-1}},\yO^{m-1}_{s_{m-1}},\dotsc,
  R^{2}_{u_{2}}, \yO^{2}_{s_{2}},\yO^{1}_{s_{1}}, \yMc)
\end{equation}
for all legitimate $m$, $u_i$, and outcomes $s_i$. Additional assumptions
are necessary to determine these probabilities. For example, the particular
order of all past outcomes, the lengths of the previous subsequences, the
robot's previous distributions, and other factors external to the
experiment might all be relevant for these probabilities. Our robot is
programmed with this simpler assumption:
\begin{itemize}
...
\begin{multline}\label{eq:def_changepoint_function}
  \yrs(v,m)  \defd \p(R^{m}_{0} \|  R^{m-1}_{v-1}, \yMc),
  \qquad 1-\yrs(v,m) \equiv  \p(R^{m}_{v} \|  R^{m-1}_{v-1}, \yMc),
  \\
  m \in\set{2,3,\dotsc,M}, \quad v\in\set{1,\dotsc,m-1}.
\end{multline}
The value $\yrs(v,m)$ is the probability that the $m$th observation starts
a new subsequence given that the previous subsequence, including the
$(m-1)$th outcome, has reached length $v$.
...
previous outcomes. This joint probability can be decomposed introducing
$R^{m-1}_{v}$ for the previous observation:
\begin{multline}
  \label{eq:prob_R_and_last_decompose}
  \p(R^{m}_{u}, \yO^{m-1}_{s_{m-1}} \|
  \yO^{m-2}_{s_{m-2}}, \dotsc, \yO^{1}_{s_{1}}, \yMc) 
 ={} \\
  \frac{\sum_{v=1}^{m-1}
...
\p(\yO^{m-2}_{s_{m-2}} \| \yO^{m-3}_{s_{m-3}}, \dotsc, \yO^{1}_{s_{1}}, \yMc)}
  ={}\\
    \frac{1}{\p(\yO^{m-2}_{s_{m-2}} \| \yO^{m-3}_{s_{m-3}}, \dotsc, \yO^{1}_{s_{1}}, \yMc)} \times
...
\item The first factor within the $v$ sum was simplified using the Markov
  property~\eqref{eq:prob_R_markov}. It is given by the changepoint
  function~\eqref{eq:def_changepoint_function}.
...
\item The last factor within the $v$ sum, the joint probability for
  $(R^{m-1}_{v-1}, \yO^{m-2}_{s_{m-2}})$, is analogous to the joint
...
\begin{table}[!p]
  ...
  \begin{tabularx}{\textwidth}{X}\hline
    ...
    \item Set $m=1$. For $s_1 \in \set{1,\dotsc,N}$ set
      \[A_1(s_1) %\defd \p(\yO^{1}_{s_{1}} \| \yMc)
        =\ynn_{s_1}, \qquad C_1(0) = 1 \]
      $A_1(s_1)$ is the probability distribution for the $1$st outcome
    \item Observe the $1$th outcome: $s_{1}$
    \item Keep  $A_1(s_1)$ for the next two steps and $C_1(0)$ for
      the next step

    \item Increase $m$ by $1$: $m=2$
      \bigskip
...
A_1(s_1) \times \begin{cases}
          \yrs(1,2) &\text{for $u=0$},\\ 1-\yrs(1,2)&\text{for $u=1$}
        \end{cases}
      \]
      \item For $s_{2} \in \set{1,\dotsc,N}$, calculate
        \[ 
          B_{2}(u,s_{2})
          % \defd \p(\yO^{2}_{s_{2}} \| R^{2}_{u}, \yO^{1}_{s_{1}}, \yMc)
          = \frac{\yN\ynn_{s_{2}}+u\yff_{s_{2}}}{\yN + u},
                \qquad
    \yff_{s_{2}} \defd\Bigl\{
    \parbox[c]{\the\myl}{\footnotesize rel. frequency of $s_2$ in\\
  the past $u$ observations}
          \]
    \end{enumerate}
  \item  For $s_2 \in \set{1,\dotsc,N}$ calculate
    \[A_2(s_2)
%      \defd  \p(\yO^{2}_{s_{2}} \| \yO^{1}_{s_{1}}, \yMc) =
      = \frac{\sum_{u=0}^{1} B_{2}(u,s_{2}) \; C_{2}(u)}{A_{1}(s_{1})} \]
          this is the probability distribution for the $2$nd outcome
    \item Observe the $2$nd outcome: $s_{2}$
    \item Keep $A_{2}(s_{2})$ for the next two steps, and $B_{2}(u,s_{2})$,
      $C_{2}(u)$, $u\in\set{0,1}$, for the next step
    \item Increase $m$ by $1$: $m=3$
\item Go to step~\ref{item:first_step} of table~\ref{tab:adamsetal_algorithm}
\end{enumerate}

\bigskip
These initial steps are equivalent to formally setting
\[ B_1(0,s_1)= \ynn_{s_1},\quad
  C_1(0) = 1,\quad
  A_0(s_0) =1
\]
and proceeding for $m=1$ from step~\ref{item:predictive_step} of
table~\ref{tab:adamsetal_algorithm} \\\hline
  \end{tabularx}
\end{table}
...
\begin{table}[!p]
  ...
  \begin{tabularx}{\textwidth}{X}\hline
    ...
    \item\label{item:first_step}For $u \in \set{0,\dotsc, m-1}$:
      \begin{enumerate}[label*=.\arabic*]
      \item Calculate
        \[ \begin{multlined}[t][0.85\textwidth]
            C_{m}(u) =
            % \defd \p(R^{m+1}_{u} , \yO^{m}_{s_{m}} \| \yO^{m-1}_{s_{m-1}}, \dotsc,
            % \yO^{1}_{s_{1}}, \yMc) ={}\\[\jot]
            \frac{1}{A_{m-2}(s_{m-2})} \times{}\\
            \begin{cases}
          \sum_{v=1}^{m-1} \yrs(v,m) \; B_{m-1}(v-1,s_{m-1}) \; C_{m-1}(v-1)
              & \text{if $u=0$},
          \\[\jot]
              [1-\yrs(u,m)]  \; B_{m-1}(u-1,s_{m-1}) \; C_{m-1}(u-1)
              & \text{if $u\ge 1$}    
      ...
      \end{multlined}
    \]
      \item For $s_{m} \in \set{1,\dotsc,N}$, calculate
        \[ %[][0.85\textwidth]
            B_{m}(u,s_{m})
            % \defd \p(\yO^{m+1}_{s_{m+1}} \| R^{m+1}_{u}, \yO^{m}_{s_{m}},
            % \dotsc, \yO^{m+1-u}_{s_{m+1-u}}, \yMc)
            = \frac{\yN\ynn_{s_{m}}+u\yff_{s_{m}}}{\yN + u},
                \qquad
    \yff_{s_{m}} \defd\Bigl\{
    \parbox[c]{\the\myl}{\footnotesize rel. frequency of $s_m$ in\\
  the past $u$ observations}
      ...
      %   \]
  \end{enumerate}
\item\label{item:predictive_step}
  For $s_{m} \in \set{1,\dotsc,N}$, calculate
  \[ %[][0.85\textwidth]
    A_{m}(s_{m})
    % \defd \p(\yO^{m+1}_{s_{m+1}} \|
    % \yO^{m}_{s_{m}}, \dotsc, \yO^{1}_{s_{1}}, \yMc) ={}\\[\jot]
=    \frac{\sum_{u=0}^{m-1} B_{m}(u,s_{m}) \;
      C_{m}(u)}{A_{m-1}(s_{m-1})}
  \]
  this is the probability distribution for the $m$th outcome
\item Observe the $m$th outcome: $s_{m}$
\item Keep $A_{m}(s_{m})$ for the next two steps, and
  $B_{m}(u,s_{m})$, $C_{m}(u)$, $u\in\set{0,\dotsc,m-1}$ for the next step
...
%% relation between old (s',m') and new (v,m):
%% m' = 1,...,M-1; s' = 0,...,m'-1
%% m = 2,...,M; v = 1,...,m-1
%% m=m'+1, v=s'+1
...
% 24
% 31 robot jumps
% 2 very diff values h

%% 40, 8, 12 for mean/std?


%% mean-js: js=0.0731+0.0502 (0.0392-0.101), km=4.83+4.72 (1.59-5.37)
%% mean-km2: js=0.0949+0.0750 (0.0481-0.122), km=3.66+3.89 (1.18-4.88)
