...
%% Last-Updated: 2018-05-28T14:39:55+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
\end{equation}
in words it states that past data are irrelevant for guessing whether the
next observation starts a new subsequence or not, given that we know how
many data belong to the current subsequence, and given that we \emph{don't}
yet know the value of the next observation.
...
discards the possibility that particular values of past observations may
 trigger the start of a new sequence. The length of the current
subsequence, $R^{m}_{s}$, is relevant, however, because the next
observation can only start a new subsequence or continue the previous one,
so that either $r=0$ or $r=s+1$:
\begin{equation}
  \label{eq:R_cond_R}
  \p(R^{m+1}_{r} \| R^{m}_{s}, \yI) =
  ...
  \end{cases}
\end{equation}
which simplifies our calculations. In our study we assume that $\yrs(s)$
either is independent of $s$ or increases with $s$, expressing our
expectation that subsequences shouldn't be too long. This last option is
reasonable if the participants were warned about changepoints during the
experiment.

The third factor is a distribution analogous to
\eqn~\eqref{eq:prob_R_and_last} but for the previous observation. In our
recursive scheme it was calculated in the previous observation and saved
into memory.
