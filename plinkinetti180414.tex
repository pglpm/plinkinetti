...
%% Last-Updated: 2018-04-17T00:13:59+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
would likely notice the change by around trial~25. An exchangeable model,
however, does not have a short \enquote{evidence memory}: the basic
assumption behind it is that there's \enquote{something} constant in all
trials; loosely speaking, a \enquote{constant mechanism}. An alternative
and maybe preferable point of view is this: an exchangeable model has
memory of past observations, but not of their order; for the robot, past
observations are shuffled, and any outcomes from narrow or wide generating
processes are mingled.
...
\textbullet\ The experimental setup is open to a huge number of analyses from the
participants' side, and it's fascinating how analyses at two very different
depths can lead to the same predictions. Consider these cases:
\begin{enumerate}
\item We can say \enquote{alright, there are 40 slots}, and just
  give a uniform distributions to the 40 possibilities.
\item We can consider the pyramidal mechanism of the game, which leads
  to a binomial distribution.
\item We can consider that this is a computerized version of the game. The
  computer could simulate the physics of the actual game. But the image of
  the mechanism could also be just for show, the computer being programmed
  to distribute the outcomes according to a predetermined, completely
  arbitrary distribution. From this point of view we could again decide to assign
  a uniform distribution.
\end{enumerate}

\medskip
\textbullet\ The wild variety of hypotheses that a participant can explore
during the task, inspired by past experiences, is also very fascinating. A
participant could suddenly wonder whether the computer program is actually
using the participant's own distribution during the trials. Motivated by
this, the participant could, say, suddenly move all probability to a slot
at the edge and check if this seems to influence the next outcome (\cf\
participant~31).

\medskip \textbullet\ De~Finetti introduced the notion of exchangeability
and proved the connected theorem in order to avoid thinking in terms of
\enquote{mechanisms} or of \enquote{true unknown distributions}. He
characterized a statistical model in terms of \emph{features of the
  predictive distribution} -- for example, that it be invariant under
exchanges of the outcomes. This point of view was taken up by many other
statisticians and has been used to characterize many other statistical
models, like Markov ones for example
\citep{freedman1962,diaconisetal1980c,zaman1984,fortinietal2002,fortinietal2014}.
This point of view can be very useful in statistics and inference, but
maybe not to understand the psychology behind prediction
\citep[\cf][\chap~3]{good1965}.
