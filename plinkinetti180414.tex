...
%% Last-Updated: 2018-05-28T16:30:53+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
\begin{multline}
  ...
  \text{\footnotesize\emph{Johnson-Dirichlet}}\left\{\;
    \!\begin{aligned}
        &\p(\ptext{data in 1st subsequence} \| \ptext{particular division}, \yI)
          \times{}\\
        &\p(\ptext{data in 2nd subsequence} \| \ptext{particular division}, \yI)
          \times{}
  \end{aligned}\right.\\
    {}\dotsm \times
  \p(\ptext{particular division} \| \yI),
\end{multline}
...
decomposed  using with the formula above:
\begin{multline}
...
The probability distribution for $R^{m+1}_{r}$ given the data represents
the robot's guess of whether the new observation starts a new subsequence
or whether it is part of the previous subsequence, and of how long that
subsequence is. This distribution is calculate recursively. First write it as
\begin{equation}
  \label{eq:prob_R_and_last}
  \p(R^{m+1}_{r} \| D^{m}_{d_{m}}, \dotsc, D^{1}_{d_{1}}, \yI) =
  \frac{
...
\end{equation}
where the denominator corresponds to the
distribution~\eqref{eq:prob_new_cond_R} calculated in the previous
observation, which we can have saved into memory. The numerator can be
decomposed introducing $R^{m}_{s}$ for the previous observation:
\begin{multline}
  ...
  \begin{aligned}[b]
  &=\sum_{s=0}^{m-1}
  \p(R^{m+1}_{r}, D^{m}_{d_{m}}, R^{m}_{s},\|
...
&\!\begin{multlined}[b][0.7\textwidth]
  ...
  \end{aligned}
\end{multline}
Three factors appear within the last sum; let's examine them.
...
sum~\eqref{eq:prob_R_and_last_decompose} when $r\ge 1$ only the term $s=r-1$
survives. In our study we assume that $\yrs(s)$ either is independent of
$s$ or increases with $s$, expressing our expectation that subsequences
shouldn't be too long. This last option is reasonable if the participants
were warned about changepoints during the experiment.

The second factor can be calculated with
formula~\eqref{eq:irrelevant_data_conditional_R}.

The third factor is a distribution analogous to
\eqn~\eqref{eq:prob_R_and_last} but for the previous observation. In our
recursive scheme it was already calculated in the previous observation and
saved into memory.
...
\[   \p(D^{m+1}_{d_{m+1}} \|
  D^{m}_{d_{m}}, \dotsc, D^{1}_{d_{1}}, \yI)
\]
is shown in table~\ref{tab:adamsetal_algorithm}. For $m=1$ some preliminary
initialization steps are necessary.

\begin{table}[p]
  \centering
  \begin{tabularx}{\textwidth}{X}\hline
    \begin{enumerate}
  \item\label{item:first_step}for $0\le r \le m$,
    \begin{enumerate}[label*=\arabic*.]
    \item using the Johnson-Dirichlet model calculate
      \[ A(r) \defd \p(D^{m+1}_{d_{m+1}} \| R^{m+1}_{r}, D^{m}_{d_{m}},
        \dotsc, D^{m+1-r}_{d_{m+1-r}}, \yI) \]
    \item for $0\le s \le m-1$,
      \begin{enumerate}[label*=\arabic*.]
      \item using the Johnson-Dirichlet model calculate
        \[ J(s) \defd \p(D^{m}_{d_{m}} \| R^{m}_{s}, D^{m-1}_{d_{m-1}}
          \dotsc, D^{m-s}_{d_{m-s}}, \yI)
        \]
      \item retrieve from memory
        \[ K(s) \defd \p(R^{m}_{s} \| D^{m-1}_{d_{m-1}} \dotsc,
          D^{1}_{d_{1}}, \yI) \]
      \end{enumerate}
    \item calculate
\[        \begin{multlined}[t][0.8\textwidth]
        K(r) \defd \p(R^{m+1}_{r}, D^{m}_{d_{m}} \| D^{m-1}_{d_{m-1}}
        \dotsc,
        D^{1}_{d_{1}}, \yI) ={}\\
        \begin{cases}
          \sum_{s=0}^{m-1} \yrs(s) \times J(s) \times K(s) & \text{if $r=0$},
          \\
          [1-\yrs(r-1)] \times J(r-1) \times K(r-1) & \text{if $r\ge 1$};    
        \end{cases}
      \end{multlined}
    \]
  \end{enumerate}
  \item retrieve from memory
    \[ C \defd \p(D^{m}_{d_{m}} \| D^{m-1}_{d_{m-1}} \dotsc, D^{1}_{d_{1}},
      \yI) \]
  \item calculate
    \[D \defd \p(D^{m+1}_{d_{m+1}} \|
      D^{m}_{d_{m}}, \dotsc, D^{1}_{d_{1}}, \yI) = \frac{\sum_{r=0}^{m}
        A(r) \times B(r)}{C}.\]
  \item save $K(r)$ and $C$ into memory
