...
robotdistribution <- function(priorfrequencies,allparams,obs){
    ...
    A1 <- 1
    C <- c(1)
    B <- matrix(priorfrequencies,nrow=1)
    ...
    for(m in 2:(n+1)){
        ...
        B <-  t(L*priorfrequencies +
                   (cumfreqs[,m]-cumfreqs[,m:1]))/(L+(0:(m-1)))
...
## old robotdistribution function
robotdistributionold <- function(priorfrequencies,allparams,obs){
    n <- length(obs)
    stubbornness <- exp(allparams[1])
    params <- allparams[-1]
    ## sequence of cumulative frequencies:
...
reducediscrepancyold <- function(participant,maxtrials,startpoints=4,seed=999){
    set.seed(seed)
...
generategraphsummary <- function(participants,savedir,label){
    ...
    for(i in participants){j <- j+1
        ...
        if(length(contents$optres$par) != 4){
            print(i)
            print(contents$optres$par)}
            mat[j,] <- c(i,t(contents$optres$par)[1:4])
    ...
    for(i in participants){j <- j+1
        ...
        if(file.exists(filename)){
            params <- readRDS(filename)$optres$par
            compres <- comparisonall(i,200,savedir=savedir,label=label,params=params,maxes=maxes)
        }}
}
...
savedistr <- function(participants,dir,label){
    ...
    for(i in participants){j <- j+1
		...
		summary <- c(i,contents$compres)
            ...
            write.table(regularizedistr(i,200),paste0(dir,'opdistr',i,label,'.csv'),sep=',',row.names=F,col.names=F,na='Infinity')
			
		write.table(summary$robotdistr,paste0(dir,'rdistr',i,label,'.csv'),sep=',',row.names=F,col.names=F,na='Infinity')
        }}
}

savepdistr <- function(participants,dir){
    for(i in participants){
		write.table(distribution(i,200),paste0(dir,'opdistr',i,'.csv'),sep=',',row.names=F,col.names=F,na='Infinity')
        }}
...
respars <- function(participant,dir,label){
    slabel <- substring(label,1,1)
    if(slabel!='' & slabel!='_'){label <- paste0('_',label)}
	slabel <- substring(dir,nchar(dir))
    if(slabel!='/'){dir <- paste0(dir,'/')}
        filename <- paste0(dir,'summary_p',participant,label,'.rds')
        if(!file.exists(filename)){
        filename <- paste0(dir,'summary_p',participant,'_NOTCONVERGED',label,'.rds')}
    readRDS(filename)$optres$par
}
